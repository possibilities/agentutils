...
describe("DocumentModel", () => {
  test("rebases non-overlapping Transactions", () => {
    ...
    const agent = model.apply({ actor: "assistant", baseRevision: base, edits: [{ start: 11, end: 16, text: "GAMMA" }] })
  ...
  test("refuses an overlap with intervening work", () => {
    ...
    expect(() =>
      model.apply({ actor: "assistant", baseRevision: base, edits: [{ start: 8, end: 10, text: "AGENT" }] }),
    ).toThrow(DomainError)
  })
  ...
  test("undo rebases around later disjoint work", () => {
    ...
    model.undo(first.id, "assistant", model.revision)
  ...
  test("serializes enough history to continue rebasing", () => {
    ...
    restored.apply({ actor: "assistant", baseRevision: base, edits: [{ start: 8, end: 13, text: "THREE" }] })
